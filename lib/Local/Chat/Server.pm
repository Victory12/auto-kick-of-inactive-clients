...
sub listen {
	...
	while () {
		...
		for my $cli (keys %{$self->peers}) {
				my $asd = time();
				p $asd;
				my $qwe = $self->peers->{ $cli }->active ;
				p $qwe;
				if ( ( time() - $self->peers->{ $cli }->active ) > 10 ){
					$self->peers->{ $cli }->disconnect ("message in time");
					delete $self->peers->{$cli};
				}
			}
	}
...
sub accept_client {
	...
	$client->on_msg(sub {
		...
		($to && $self->rooms->{$to}) ? $room = $self->rooms->{$to} : $room = $self->default_room;
