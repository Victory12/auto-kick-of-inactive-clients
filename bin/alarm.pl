...
my $server = Local::Chat::ServerConnection->new(nick => $nick, host => $ARGV[0] || 'localhost', 
	...
	on_message => sub {
		...
		if ($message->{text} =~ m!^alarm ((\d{4})[/:-](\d\d)[/:-](\d\d) (\d\d):(\d\d):(\d\d).+)$! ) {
			if ($2 > 1970 && $3 <= 12 && $3 >= 1 && $4 <= 31 && $5 < 24 && $6 < 60 && $7 < 60) {
				my $time = Date::Calc::Date_to_Time($2, $3, $4, $5, $6, $7);
				...
				@reminders = sort { %$a{'time'} <=> %$b{'time'} } @reminders;
			}
		}
		if ($message->{text} =~ m/^!who/) {
			$srv->message({ text => 'i am alarm' });
		}
	...
	on_idle => sub {
		my ($srv, $message) = @_;
		if (defined $reminders[0] and $reminders[0]->{'time'} - (time + 3600*3) <= 0) {
			use DDP;
			p @reminders;
			my $r = shift @reminders;
			$srv->message({ text => ($r->{nick}.' '.$r->{message}) });
		}
	}
);
